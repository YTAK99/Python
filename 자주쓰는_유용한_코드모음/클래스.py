...
''' 클래스''' # a.k.a. 붕어빵 틀과 붕어빵들
클래스를 쓰는 핵심 이유 3가지
- 캡슐화 (묶기): 변수(이름, 체력)와 함수(공격하기)가 흩어져 있지 않고 Marine이라는 주머니 안에 깔끔하게 들어있다.

- 재사용성: 설계도 하나만 잘 만들어두면, 마린을 1,000마리 만들어도 코드는 똑같이 깔끔하다.

- 유지보수: 만약 모든 마린의 공격 방식을 바꾸고 싶다면? 
           클래스 안의 attack 함수 딱 한 곳만 수정하면 수천 마리의 마린에게 한 번에 적용된다.
...
def add(num1, num2):             # 전달값

add(1,2):           # 함수 호출


class BlackBox:
    def set_travel_mode(self, min):           # 클래스 메소드(전달값1, 전달값2)
